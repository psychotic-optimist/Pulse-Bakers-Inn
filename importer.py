...
def parse_all_sheets(
    ...
    # Bread orders — Local routes only.
    # Freighter (depot) rows are excluded here so that a re-import never
    # overwrites manually-entered Freighter orders in the database.
    bread, warn = parse_order_sheet(io.BytesIO(file_obj.getvalue()), dispatch_date, "Orders")
    local_only = [o for o in bread if o.get("route_type") != "Freighter"]
    all_orders.extend(local_only)
    all_warnings.extend(warn)

    if len(bread) != len(local_only):
        skipped = len(bread) - len(local_only)
        all_warnings.append(
            f"Note: {skipped} Freighter row(s) in the Excel sheet were skipped — "
            "Freighter orders must be entered manually in the app."
        )

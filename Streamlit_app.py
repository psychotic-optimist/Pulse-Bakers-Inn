...
def render_board_tab(
    ...
    # Drop rows where nothing was ordered and nothing was loaded
    augmented = [o for o in augmented if o.get("target_qty", 0) > 0 or o.get("loaded_qty", 0) > 0]

    if not augmented:
        st.info(f"No {tab_label} orders with quantities for today.")
        return
...
CONFECT_SKUS_ORDER = [
    "MEGA CREAM BUN", "SUGAR BUNS (TOTAL)", "UNBRANDED MEGA", "UNBRANDED SUGAR",
    "BEEF SAUSAGE ROLLS", "CHOC CREAM DOUGHNUT", "HOT DOG ROLLS",
    "HAMBURGER ROLLS(PACKETS)", "HOT DOG ROLLS(PACKETS)", "SAMPLES",
    "PLAIN HAMBURGER", "SESAME", "BEEF SAUSAGE ROLLS ",
    "YELLOW SUGAR BUNS", "BISCUITS ", "SCONES", "ROCK BUNS",
    "munchies 150g", "1ks munchies ", "2kg munchies",
]

_DEPOT_TABLE_CSS = """
<style>
.depot-sku-table { width:100%; border-collapse:collapse; font-size:0.9rem; }
.depot-sku-table th {
    background:#1B2D6B; color:#C9A84C;
    padding:6px 10px; text-align:center;
    border:1px solid #2d3f7a; font-size:0.82rem;
    text-transform:uppercase; letter-spacing:0.4px;
}
.depot-sku-table th.sku-col { text-align:left; }
.depot-sku-table td {
    padding:5px 10px; text-align:center;
    border:1px solid #e2e8f0; color:#9ca3af; font-size:0.88rem;
}
.depot-sku-table td.sku-label { text-align:left; font-weight:500; color:#1e293b; }
.depot-sku-table td.qty-pos { font-weight:700; color:#1B2D6B; }
.depot-sku-table tr.total-bread-row td {
    background:#1B2D6B !important; color:#ffffff !important;
    font-weight:700;
}
.depot-sku-table tr:nth-child(even):not(.total-bread-row) td { background:#f8fafc; }
.depot-slide-header {
...
def _build_depot_sku_table(
    ...
    idx: dict[tuple, dict] = {}
    all_skus_in_data: list[str] = []
    seen_skus: set = set()
    for r in rows:
        key = (r["sku_name"], r["truck_label"])
        idx[key] = r
        if r["sku_name"] not in seen_skus:
            seen_skus.add(r["sku_name"])
            all_skus_in_data.append(r["sku_name"])

    # Build ordered SKU list: bread first (fixed order), then confect
    sku_order: list[str] = []
    for s in BREAD_SKUS_ORDER:
        if s in seen_skus:
            sku_order.append(s)
    # Add any bread SKUs in data not in our predefined order
    for s in all_skus_in_data:
        if s not in sku_order and idx.get((s, truck_labels[0] if truck_labels else ""), {}).get("sku_group") == "bread":
            sku_order.append(s)
    for s in CONFECT_SKUS_ORDER:
        if s in seen_skus:
            sku_order.append(s)
    # Catch anything remaining
    for s in all_skus_in_data:
        if s not in sku_order:
            sku_order.append(s)

    n_trucks = len(truck_labels)
    col_width = max(10, int(60 / max(n_trucks, 1)))

    # ── Header ──────────────────────────────────────────────────────────
    header = f"<th class='sku-col' style='width:30%'>PRODUCT DESCRIPTION</th>"
    for tl in truck_labels:
        reg = truck_regs.get(tl, "")
        reg_line = f"<br><small style='color:#C9A84C;font-size:0.75em'>{reg}</small>" if reg else ""
        header += f"<th style='width:{col_width}%'>{tl}{reg_line}</th>"

    # ── Rows ────────────────────────────────────────────────────────────
    body = ""
    bread_totals = {tl: 0 for tl in truck_labels}

    # Separate bread and confect
    bread_skus  = [s for s in sku_order if s.upper() in {x.upper() for x in BREAD_SKUS_ORDER} or
                   any(idx.get((s, tl), {}).get("sku_group") == "bread" for tl in truck_labels)]
    ...
    for sku in bread_skus:
        ...
        for tl in truck_labels:
            r = idx.get((sku, tl))
            qty = r["ordered_qty"] if r else 0
            bread_totals[tl] += qty
            if show_loaded and r:
                loaded = r.get("loaded_qty", 0)
                cls = "qty-pos" if qty > 0 else ""
                cells += f"<td class='{cls}'>{qty:,}<br><small style='color:#22c55e'>{loaded:,}</small></td>"
            else:
                cls = "qty-pos" if qty > 0 else ""
                cells += f"<td class='{cls}'>{qty:,}</td>"
        body += f"<tr><{cells}</tr>"

    # TOTAL BREAD row
    total_bread_cells = "<td class='sku-label'><b>TOTAL BREAD</b></td>"
    for tl in truck_labels:
        t = bread_totals[tl]
...
def render_tv_mode(orders: list[dict], settings: dict) -> None:
    ...
    Slides 1…N: one slide per depot (SKU breakdown table)
    Last slide: Local routes board
    Rotates every 15 s.  Data refreshes every 30 s via st_autorefresh.
    """
    ...
    with slide_placeholder.container():
        ...
        # ── Slides 1…N: per-depot SKU breakdown ────────────────────────
            ...
            for tl in truck_labels:
                ...
                if remaining <= 0:
                    ...
                    from datetime import timedelta
                    etc_dt = tv_now + timedelta(hours=remaining / DEPOT_LOADING_RATE)
                    all_etcs.append(etc_dt)
                    truck_etcs[tl] = _fmt_etc(etc_dt, tv_now)

            route_etc_str = (
                "All trucks done"
                if not all_etcs
                else "Done by " + _fmt_etc(max(all_etcs), tv_now)
            )
            banner_color = "#166534" if not all_etcs else "#1B2D6B"

            # Progress
            total_ordered = depot_summary["total_ordered"]
            total_loaded  = depot_summary["total_loaded"]
            ...
            # Progress bar + ETC banner
            st.markdown(
                f"<div style='margin-top:0.75rem;background:#f0f4ff;"
                f"border-left:5px solid {banner_color};"
            ...
            st.markdown(_progress_bar_html(pct), unsafe_allow_html=True)
            st.markdown(
                f"<small>{total_loaded:,} / {total_ordered:,} loaves loaded &nbsp; ({pct:.0f}%)</small>",
                unsafe_allow_html=True,
            )
        ...
        # ── Last slide: Local routes ────────────────────────────────────
            ...
            locals_non_empty = [o for o in locals_ if o.get("target_qty", 0) > 0 or o.get("loaded_qty", 0) > 0]
            render_board_tab(locals_non_empty, hourly_rate, True, "Local")
    ...
    st.markdown(
        f"<div class='tv-slide-indicator'>{dots} &nbsp; <b>{current_label}</b> "

...
def render_tv_mode(orders: list[dict], settings: dict) -> None:
    ...
    with slide_placeholder.container():
        if slide_index == 0:
            ...
            # ── Slide 2: Loading Plan (depot / freighter trucks only) ───
            st.markdown("### 📋 Loading Plan — Freighters / Depots")
            if freighters:
                augmented = calculations.augment_orders(freighters, hourly_rate)
                rows_html = ""
                for o in augmented:
                    rows_html += (
                        f"<tr>"
